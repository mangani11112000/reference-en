...
[float]
=== Notes and Warnings
The PWM outputs frequency generated on pins 5 and 6 will have higher value(double) with respect to the other PWM pins. This is because of interactions with the `millis()` and `delay()` functions, which share the same internal timer used to generate those PWM outputs. This will be noticed mostly on low duty-cycle settings (e.g. 0 - 10) and may result in a value of 0 not fully turning off the output on pins 5 and 6.
